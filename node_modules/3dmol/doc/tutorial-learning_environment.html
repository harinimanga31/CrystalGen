...
python3 server.py
</code></pre><p>By default, this will launch a server on port 5000, e.g. <code>http://HOSTNAME:5000/</code>. The viewer is at <code>http://HOSTNAME:5000/static/viewer.html</code>.<br>Your network must allow external connections to the specified port.<br>Do <b>not</b> run this command as superuser to run the server on the privileged default webserver port 80 without consulting with a network security specialist.</p><p>Note that files put in the <code>3dmol</code> directory can be referenced as relative paths within the <code>url=</code> component. For example: <code>http://localhost:5000/static/viewer.html?url=molecule.sdf&amp;style=stick</code></p><p>If you already have a webserver running, you can redirect the websocket requests used by 3Dmol.js to the server running on port 5000. For example, for apache the following is added to the configuration (see <a href="https://stackoverflow.com/questions/36472920/apache-proxy-configuration-for-socket-io-project-not-in-root">here</a>):</p><pre class="prettyprint source lang-apache.conf"><code>RewriteEngine On
